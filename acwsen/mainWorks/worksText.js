//*************************유의사항**************************//
// 1. "" 큰따옴표 사용불가(작은 따옴표''는 가능)
// 2. 줄바꿈 불가

// 사업 시작
// 첫번째 사업 시작
$(document).ready(function () {
  // 제목1
  $('#workTitle1').text(
    'Institute Programs in the Humanities & Social Sciences'
  ),
    // 요약1
    $('#workSum1').text(
      'Race and Gender: Global Korea, Neo-Racialization and Intersectionality'
    ),
    // 링크1
    $('.link1').attr(
      'href',
      'https://cms.ewha.ac.kr/user/indexSub.action?codyMenuSeq=79524375&siteId=acwsen&menuUIType=sub'
    ),
    // 두번째 사업 시작
    // 제목2
    $('#workTitle2').text(
      'Research and Promotion Project of Modern Female Figures in the Field of Culture '
    ),
    // 요약2
    $('#workSum2').text(
      'Women Pathfinder: Modern Female Figures in the Field of Culture'
    ),
    // 링크2
    $('.link2').attr(
      'href',
      'https://cms.ewha.ac.kr/user/indexSub.action?codyMenuSeq=59020242&siteId=acwsen&menuUIType=sub'
    ),
    // 세번째 사업 시작
    // 제목3
    $('#workTitle3').text(
      "Asian Women's Studies Higher Curriculum Development Project"
    ),
    // 요약3
    $('#workSum3').text(
      'Leadership Development Program on Women’s Human Rights for Higher Education in Asia'
    ),
    // 링크3
    $('.link3').attr(
      'href',
      'http://cms.ewha.ac.kr/user/indexSub.action?codyMenuSeq=59020383&siteId=acwsen&menuUIType=sub'
    );
  //세번째 슬라이드 끝
});
// 슬라이드 끝
